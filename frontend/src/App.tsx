...
import { createBatchJobs, fetchHealth, fetchHistory, fetchJob } from './lib/api'
...
export function App() {
  ...
  async function openHealthModal() {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : 'API 상태를 확인하지 못했습니다.'
  ...
  async function submit() {
    ...
    try {
      ...
      const rejected = data.rejected?.length ? `, 거부 ${data.rejected.length}개` : ''
      setMessage(`변환 job ${data.accepted_count}개를 생성했습니다${rejected}.`)
